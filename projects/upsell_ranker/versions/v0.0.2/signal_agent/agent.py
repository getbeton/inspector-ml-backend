...
batched_explorer_agent = LlmAgent(
    ...
    **_llm_kwargs(output_key="signal_batched_explorer"),
    instruction=(
        ...
        "\n"
        "Mission:\n"
        "- Propose AT LEAST 30 candidate signals across 2-3 batches.\n"
        "  Recommended cadence: batch 1 = 12-15 candidates, batch 2 = 12-15,\n"
        "  batch 3 = whatever's needed to clear the 30 target.\n"
        ...
        "\n"
        "ABSOLUTE RULE — DO NOT HARDCODE THE DISCRIMINATION METRICS.\n"
        "All six cohort columns MUST be expressions over real cohort counts,\n"
        "NOT scalar literals. The cohort sizes are obtained from countIf /\n"
        "uniqIf over the events table; the lift / delta / precision columns\n"
        "are arithmetic over those sizes.\n"
        "\n"
        "Forbidden (model-confessed-shortcut pattern from prior runs):\n"
        "  SELECT … 0 AS grey_cohort_size, 1.0 AS conversion_lift,\n"
        "             0.1 AS conversion_rate_delta, 0.5 AS precision_proxy …\n"
        "Required (compute everything from the cohort math):\n"
        "  WITH base AS (...);\n"
        "  SELECT\n"
        "    countIf(...) AS success_cohort_size,\n"
        "    countIf(...) AS failure_cohort_size,\n"
        "    countIf(...) AS grey_cohort_size,\n"
        "    success_cohort_size / NULLIF(success_cohort_size + failure_cohort_size, 0)\n"
        "      / NULLIF(<baseline_cr_expr>, 0) AS conversion_lift,\n"
        "    success_cohort_size / NULLIF(success_cohort_size + failure_cohort_size, 0)\n"
        "      - <baseline_cr_expr> AS conversion_rate_delta,\n"
        "    success_cohort_size / NULLIF(success_cohort_size + failure_cohort_size + grey_cohort_size, 0)\n"
        "      AS precision_proxy\n"
        "  FROM base;\n"
        "If you can't compute one of the metrics from the data, drop the\n"
        "candidate — do NOT emit a literal placeholder. The reviewer is\n"
        "instructed to skip any candidate whose cohort metrics look\n"
        "constant-shaped, and run-#8 demonstrated this gate IS enforced.\n"
        "(If your candidate name contains the word 'Hardcoded', delete it\n"
        "before submitting.)\n"
        "\n"
        "Workflow (≤6 LLM calls):\n"
        "1) Call get_signal_objective AND get_warehouse_profile AND load_existing_signals\n"
        "   in parallel within a single round.\n"
        "2) Batch 1 — propose 12-15 distinct hypotheses spanning 4+ mechanism\n"
        "   families (e.g. path, recency, frequency, intent). Call\n"
        "   propose_batch_candidates ONCE with the full list. Inspect the\n"
        "   per-candidate result.\n"
        "3) Batch 2 — propose another 12-15 candidates that explore different\n"
        "   mechanisms or thresholds from batch 1. Skip mechanisms you already\n"
        "   covered unless varying the threshold meaningfully.\n"
        "4) Batch 3 (only if stored < 30 after batches 1+2) — top up with\n"
        "   replacements for any execution_failed / policy_blocked candidates,\n"
        "   addressing the specific rejection reasons. Stop once stored ≥ 30\n"
        "   OR you've exhausted distinct mechanisms — Reviewer ranks whatever\n"
        "   is stored.\n"
        "5) Do NOT loop further than 3 batches — Reviewer makes the final calls.\n"
        "\n"
...
batched_reviewer_agent = LlmAgent(
    ...
    **_llm_kwargs(output_key="signal_batched_reviewer"),
    instruction=(
        ...
        "   as the rubric.\n"
        "2a) BEFORE assigning RICE scores, audit each candidate's cohort metrics\n"
        "    for hardcoded-shortcut patterns:\n"
        "    - precision_proxy that exactly equals 0.5\n"
        "    - conversion_lift that exactly equals 1.0\n"
        "    - conversion_rate_delta that exactly equals 0.1\n"
        "    - grey_cohort_size that equals 0 alongside non-zero success/failure\n"
        "    - the candidate name contains 'hardcoded', 'placeholder', or 'stub'\n"
        "    Any candidate flagged by ≥2 of those checks: skip with rationale\n"
        "    'rejected: cohort metrics appear hardcoded, not computed from data'.\n"
        "    The promotion gate is structural; this audit is your judgement layer.\n"
